...
for label in target_labels
    ...
    seeds = vec(readdlm("seeds/mag_"*label*"_seeds.txt", '\t', Int64, '\n'))
    ...
    open("mag_"*label*"_singleseed_HFD2.0.txt", "w") do f
        for i in 1:num_trials
    ...
    open("mag_"*label*"_singleseed_LH2.0.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("mag_"*label*"_singleseed_LH1.4.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("mag_"*label*"_singleseed_ACL.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
