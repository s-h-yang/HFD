...
for label in target_labels
    target_cluster = vec(readdlm("datasets/mag-dual/mag_"*label*"_papers.txt", '\t', Int64, '\n'))
    ...
    for i in 1:num_trials
        ...
        cluster, COND[i], _, _ = ucHFD(H, Delta, sigma=sigma, max_iters=100, p=2, r=r, s=s)
        PR[i], RE[i], F1[i] = compute_f1(cluster, target_cluster)
        @printf("Cluster %s, trial %d, method HFD2.0, Pr %.4f, Re %.4f, F1 %.4f, Cond %.4f\n", label, i, PR[i], RE[i], F1[i], COND[i])
    end
    open("mag_"*label*"_multiseed_HFD2.0.txt", "w") do f
        for i in 1:num_trials
    ...
    for i in 1:num_trials
        ...
        fill!(Delta, 0.0)
        Delta[seeds] .= 3*vol_T*H.degree[seeds]/vol_S
        cluster, COND[i], _, _ = ucHFD(H, Delta, sigma=sigma, max_iters=100, p=4, r=r, s=s)
        PR[i], RE[i], F1[i] = compute_f1(cluster, target_cluster)
        @printf("Cluster %s, trial %d, method HFD4.0, Pr %.4f, Re %.4f, F1 %.4f, Cond %.4f\n", label, i, PR[i], RE[i], F1[i], COND[i])
    end
    open("mag_"*label*"_multiseed_HFD4.0.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    for i in 1:num_trials
        ...
        @printf("Cluster %s, trial %d, method LH2.0, Pr %.4f, Re %.4f, F1 %.4f, Cond %.4f\n", label, i, PR[i], RE[i], F1[i], COND[i])
    end
    open("mag_"*label*"_multiseed_LH2.0.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    for i in 1:num_trials
        ...
        COND[i], cluster = hyper_sweepcut(G.H,x,G.deg,G.delta,0.0,G.order,nseeds=num_seeds)
        PR[i], RE[i], F1[i] = PRF(target_cluster,cluster)
        @printf("Cluster %s, trial %d, method LH1.4, Pr %.4f, Re %.4f, F1 %.4f, Cond %.4f\n", label, i, PR[i], RE[i], F1[i], COND[i])
    end
    open("mag_"*label*"_multiseed_LH1.4.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("mag_"*label*"_multiseed_ACL.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
