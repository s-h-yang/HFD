...
for label in target_labels
    ...
    for i in 1:num_trials
        ...
        Delta[seeds] .= 10*vol_T*H.degree[seeds]/vol_S
    ...
    open("trivago_c"*string(label)*"_multiseed_HFD2.0.txt", "w") do f
        for i in 1:num_trials
    ...
    open("trivago_c"*string(label)*"_multiseed_HFD4.0.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("trivago_c"*string(label)*"_multiseed_LH2.0.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("trivago_c"*string(label)*"_multiseed_LH1.4.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
