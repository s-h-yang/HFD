...
# Run experiments
for label in target_labels[1:end]
    ...
    open("trivago_c"*string(label)*"_UHFD.txt", "w") do f
        for i in 1:num_trials
    ...
    for i in 1:num_trials
        Delta = zeros(Float64, H.nv)
        Delta[seeds[i]] = 3*vol_T
        cluster, COND[i], _, _ = cardHFD(H, Delta, sigma=sigma, max_iters=200)
        PR[i], RE[i], F1[i] = compute_f1(cluster, target)
        @printf("Cluster %d, trial %d, method C-HFD, Pr %.4f, Re %.4f, F1 %.4f, Cond %.4f\n", label, i, PR[i], RE[i], F1[i], COND[i])
    end
    open("trivago_c"*string(label)*"_CHFD.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("trivago_c"*string(label)*"_ULH2.0.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("trivago_c"*string(label)*"_ULH1.4.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("trivago_c"*string(label)*"_CLH2.0.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("trivago_c"*string(label)*"_CLH1.4.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
    ...
    open("trivago_c"*string(label)*"_ACL.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
