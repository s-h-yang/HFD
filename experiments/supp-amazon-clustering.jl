...
G = LH.graph(M["H"], 1.0) # graph data structure for LH
...
for (c,label) in enumerate(target_labels)
    ...
    for i in 1:num_trials
        ...
        cluster, COND[i], _, _, _ = ucHFD(H, Delta, sigma=sigma, max_iters=30, r=r, s=s)
        PR[i], RE[i], F1[i] = compute_f1(cluster, target_cluster)
        @printf("Cluster %d, trial %d, method U-HFD-2.0, Pr %.4f, Re %.4f, F1 %.4f, Cond %.4f\n", label, i, PR[i], RE[i], F1[i], COND[i])
    end
    open("amazon_c"*string(label)*"_multiseed_UHFD2.0.txt", "w") do f
    for i in 1:num_trials
    ...
    for i in 1:num_trials
        ...
        fill!(Delta, 0.0)
        Delta[seeds] .= deltas[c]*vol_T*H.degree[seeds]/vol_S
        cluster, COND[i], _, _, _ = ucHFD(H, Delta, sigma=sigma, max_iters=30, p=4, r=r, s=s)
        PR[i], RE[i], F1[i] = compute_f1(cluster, target_cluster)
        @printf("Cluster %d, trial %d, method U-HFD-4.0, Pr %.4f, Re %.4f, F1 %.4f, Cond %.4f\n", label, i, PR[i], RE[i], F1[i], COND[i])
    end
    open("amazon_c"*string(label)*"_multiseed_UHFD4.0.txt", "w") do f
    for i in 1:num_trials
        @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
    end
    ...
    open("amazon_c"*string(label)*"_multiseed_ULH2.0.txt", "w") do f
    for i in 1:num_trials
        @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
    end
    ...
    open("amazon_c"*string(label)*"_multiseed_ULH1.4.txt", "w") do f
    for i in 1:num_trials
        @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
    end
