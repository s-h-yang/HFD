...
for (c,label) in enumerate(target_labels)
    ...
    seeds = readdlm("../datasets/seeds/amazon_c"*string(label)*"_seeds.txt", '\t', Int64, '\n')
    ...
    open("amazon_c"*string(label)*"_UHFD.txt", "w") do f
    for i in 1:num_trials
    ...
    open("amazon_c"*string(label)*"_ULH2.0.txt", "w") do f
    for i in 1:num_trials
        @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
    end
    ...
    open("amazon_c"*string(label)*"_ULH1.4.txt", "w") do f
    for i in 1:num_trials
        @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
    end
    ...
    open("amazon_c"*string(label)*"_ACL.txt", "w") do f
        for i in 1:num_trials
            @printf(f, "%.4f\t%.4f\t%.4f\t%.4f\n", PR[i], RE[i], F1[i], COND[i])
        end
    end
